...
async function postMoviment() {
    ...
    const req = await fetch(`${SERVER_URL}/inGame/${localGame.gameId}/${localColor}`, {
                method: 'POST',
                headers: {
...
async function getOnlineGame() {
        const res = await fetch(`${SERVER_URL}/inGame/${localGame.gameId}/${localColor}`, {
        method: 'GET',
    })
